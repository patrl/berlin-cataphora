...
\ex. Everyone who wanted \textsc{Kriszta} to read it printed out their dissertation, and\\
everyone who wanted \textsc{Robyn} to \elide{read it} printed out their essay.
...
\begin{frame}{Our idea}
  ...
  \begin{itemize}
      ...
      \item How do we account for the ability of definites to bind to their left without dispensing with the core results of dynamic semantics in the domain of anaphora with indefinite antecedents.

      \item Our claim: unlike orthodox dynamic binding of a definite by an indefinite, \textit{cataphora} involves binding by a \textit{presupposition}.

      \item We can't make sense of this in orthodox dynamic theories, so we provide an extension of DPL in which we can cash this out.
...
\begin{frame}
  ...
  \begin{itemize}
      ...
      \item In \(\displaystyle\frac{\phi}{\psi}\), \(\phi\) represents the presupposition, and \(\psi\) represents the at-issue meaning.

      \item Crucially, we take both the at-issue meaning and the presupposition are dynamic statements.

      \item See \citet{elliottSudoLenls,elliottSudoCat} for a formalisation of the analysis in terms of Dynamic Predicate Logic (\citealt{groenendijk_dynamic_1991}). Here we present the informal idea.
...
\begin{frame}{Definites}
  ...
  $$\displaystyle\frac{\exists ! a[\ml{newBook}\,x] \wedge x = a}{\ml{soldOut}\,x}$$

\end{frame}

\begin{frame}{Extension to names and pronominals}

  Similarly, we assume that proper names and pronominals also have existential presuppositions.

  \ex. $\text{Paul$^{a}_{x}$ sat down} \rightsquigarrow \displaystyle\frac{\exists ! a[a = x] \wedge x = Paul}{\ml{satDown}\,x}$

  \ex. $\text{He$^{a}_{x}$ sat down} = \displaystyle\frac{\exists ! a[x = a]}{\ml{satDown}\,x}$
